...
int findArticle(billList* list)
{
	...
	scanf(" %60s", name2);
	printf("\nUnesite pocetni datum (YYYY-MM-DD): ");
	scanf(" %19s", fromDate);
	printf("\nUnesite krajnji datum (YYYY-MM-DD): ");
	scanf(" %19s", tillDate);
...
int mostCommon(billList* list)
{
	...
	printf("\nNajcesci artikl je %s, a kolicina je %d\n", maxName, maxAmount);
...
int rarest(billList* list)
{
	...
	printf("\nNajrijedi artikl je %s, a kolicina je %d\n", minName, minAmount);
...
{
	articleList*global= listForArticles();
	Bill p = list->First;
